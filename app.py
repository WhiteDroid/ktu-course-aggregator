...
# --- CSS INJECTION FOR DYNAMIC THEME ---
if st.session_state.theme == 'light':
    ...
    chart_font_color = "#262730"
    radar_fill = 'rgba(0, 128, 128, 0.4)'
else:
...
# --- 2. DATABASE SETUP & UPGRADE (UNCHANGED) ---
...
def seed_initial_data():
    ...
    if c.fetchone()[0] == 0:
        # (Seeding logic remains identical to your original code)
...
def generate_smart_summary(sentiment_score, category):
    if category == "College":
        if sentiment_score >= 0.85: return "🌟 **AI Summary:** Students rate this institution highly (85% Positive). Excellent academics and faculty support."
        elif sentiment_score > 0.7: return "✅ **AI Summary:** Overall, students praise this institution. Strong placements make it a top choice."
        elif sentiment_score > 0.4: return "⚖️ **AI Summary:** Mixed feelings. Academics hold up, but facilities are common complaints."
        else: return "⚠️ **AI Summary:** Proceed with caution. Significant negative sentiment regarding management."
    else:
        if sentiment_score >= 0.85: return "🌟 **AI Summary:** This subject receives a very high 85% satisfaction score. Manageable syllabus."
        elif sentiment_score > 0.6: return "✅ **AI Summary:** Students find this subject rewarding and relatively easy to score."
        else: return "⚠️ **AI Summary:** This is a notorious 'filter' subject. Expect a massive workload."

def plot_gauge(score, title):
    fig = go.Figure(go.Indicator(
        mode = "gauge+number",
        value = score * 100,
        title = {'text': title, 'font': {'size': 18, 'color': chart_font_color}},
        number = {'suffix': "/100", 'font': {'color': chart_font_color}},
        gauge = {
            'axis': {'range': [None, 100], 'tickcolor': chart_font_color},
            'bar': {'color': "#00CC96"},
            'steps': [
    ...
    fig.update_layout(height=250, margin=dict(l=10, r=10, t=40, b=10), paper_bgcolor="rgba(0,0,0,0)", font={'color': chart_font_color})
...
def plot_radar(metrics):
    ...
    fig.update_traces(fill='toself', fillcolor=radar_fill, line_color='#00CC96')
    fig.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)", polar=dict(bgcolor="rgba(0,0,0,0)", radialaxis=dict(visible=False)), font={'color': chart_font_color}, margin=dict(l=30, r=30, t=30, b=30))
    st.plotly_chart(fig, use_container_width=True)

def plot_wordcloud(reviews_df):
    if not reviews_df: return
    text = " ".join([r['review_text'] for r in reviews_df])
    # Adjust colormap based on theme
    cmap = "Greens" if st.session_state.theme == 'dark' else "viridis"
    wordcloud = WordCloud(width=800, height=400, background_color=None, mode="RGBA", colormap=cmap).generate(text)
...
st.caption(f"Powered by NLP Analytics | Theme: {st.session_state.theme.capitalize()}")
st.divider()
...
with tab1:
    ...
    with col2:
        if selected_college:
            ...
            st.divider()
            display_reviews = reviews if not search_query else [r for r in reviews if search_query.lower() in r['review_text'].lower()]
            st.subheader(f"📖 Discussions ({len(display_reviews)})")
            for r in display_reviews[:10]: 
                with st.container():
                    st.markdown(f"**\"{r['review_text']}\"**")
                    c1, c2 = st.columns([1, 6])
                    with c1:
                        if st.button(f"👍 {r['upvotes']}", key=f"upcol_{r['id']}"):
                            upvote_review(r['id'])
                            st.rerun()
                    with c2:
...
with tab2:
    ...
    with c_left:
        ...
        with st.form(key="form_course"):
            new_c_review = st.text_area("Share your experience...")
            if st.form_submit_button("Submit Review"):
                if new_c_review:
    ...
    with c_right:
        ...
        st.divider()
        display_course_reviews = course_reviews if not search_query else [r for r in course_reviews if search_query.lower() in r['review_text'].lower()]
        for r in display_course_reviews[:10]:
            with st.container():
                st.markdown(f"**\"{r['review_text']}\"**")
                c1, c2 = st.columns([1, 6])
                with c1:
                    if st.button(f"👍 {r['upvotes']}", key=f"upcrs_{r['id']}"):
                        upvote_review(r['id'])
                        st.rerun()
                with c2:
                    if r['tags']: st.caption(f"Tags: {r['tags']}")
