...
def plot_radar(metrics):
    ...
    fig.update_traces(fill='toself', fillcolor='rgba(0, 204, 150, 0.5)', line_color='#00CC96')
    fig.update_layout(height=300, paper_bgcolor="rgba(0,0,0,0)", polar=dict(bgcolor="rgba(0,0,0,0)", radialaxis=dict(visible=False)), margin=dict(l=30, r=30, t=30, b=30))
    st.plotly_chart(fig, use_container_width=True)

def plot_wordcloud(reviews_df):
    if not reviews_df: return
    text = " ".join([r['review_text'] for r in reviews_df])
    wordcloud = WordCloud(width=800, height=400, background_color="rgba(0,0,0,0)", mode="RGBA", colormap="Greens").generate(text)
